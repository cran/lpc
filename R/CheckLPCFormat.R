CheckLPCFormat <- function(dat,type,soft.thresh,u){
  if(type!="survival" && type!="regression" && type!="two class") stop("type must be one of 'regression', 'two class', or 'survival'")
  ...
  if(!is.null(u) && (!is.matrix(u) || nrow(u)!=n)) stop("u must be a matrix with the same number of rows as dat$x. Remember, it can be computed automatically if you prefer (let 'u=NULL').")
} 
